...
function generateImgurUrl() {
    ...
    const randomId = Array.from({ length: 5 }, () => chars[Math.floor(Math.random() * chars.length)]).join("") + "b";
    return `https://i.imgur.com/${randomId}.png`;
}
async function getRandomImage(message) {
    // Launch Puppeteer browser
    const browser = await puppeteer_1.default.launch({ headless: false, slowMo: 50 });
